...
def virtual_screen_1_target(smiles, target_name, target_fasta, result_folder_ = "./result_test/"):
    ...
    model = models.model_pretrained(path_dir = '/home/databank/yqyang/D3SimilarityV2.0/DeepPurpose_DeepPurpose/result_3/fold_3_model')
    models.virtual_screening(smiles, target_fasta, 
                     model, drug_names = smiles, target_names = target_name, result_folder = result_folder_,
                     convert_y = False)
    # for i in range(len(smiles)):
...
def main():
    ...
    virtual_screen_all_target(mols, target_names, target_fastas)
