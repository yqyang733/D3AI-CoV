...
class Common_model(nn.Module):
    ...
    def forward(self, smiles,fasta):
        ...
        smiles_vector = self.smi_emb(smiles)    #in:[smi_nums,smi_max_len] out:[smi_nums,smi_max_len,emb_size]
        #使用的是二维卷积，需要有一维度表示通道数，所有要用扩展
        smiles_vector = torch.unsqueeze(smiles_vector,1)           #out:[smi_nums,1,smi_max_len,emb_size]
        ...
        return smile_common,fasta_common
    ...
    def _block(self, x):
        ...
        x = self.padding1(px)        #收尾补2个零，调用self.conv,维度不变。这里还是在序列长度方向提取邻域特征
        x = F.relu(x)
        x = self.conv(x)
